...
public class RedParticle extends AbstractParticle{

	public RedParticle(PerspectiveCamera camera) {
		super(camera);
		
		//Cria as partículas vermelhas do jogo
		...
		AssetManagement.assetManager.load("particles/cylinder_red2.p", ParticleEffect.class, loaderParam);
		AssetManagement.assetManager.finishLoading();
		
		effect = AssetManagement.assetManager.get("particles/cylinder_red2.p", ParticleEffect.class).copy();
		effect.init();
		Vector3 translation = new Vector3(0.0f, 0.45f, 4.6f);
